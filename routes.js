...
router.get('/cause-areas', (req, res) => {
  res.json({ categories: db.CAUSE_AREA_CATEGORIES, links: db.FUND_LINKS });
});
...
router.put('/me', requireAuth, (req, res) => {
  const { donation_amount, is_public, display_name, info_url, items } = req.body;
  ...
  db.saveAllocation(req.user.id, donation_amount, !!is_public, display_name || '', info_url || '', items);
